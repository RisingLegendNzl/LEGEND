...
function prepareDataForLSTM(historyData, historicalStreakData) {
    ...
    if (validHistory.length < TRAINING_MIN_HISTORY) { // Changed to TRAINING_MIN_HISTORY for overall dataset size
    ...
    // Also check for sequence length specific to prediction preparation
    if (validHistory.length < SEQUENCE_LENGTH) { // Need at least SEQUENCE_LENGTH for prediction to make sense
        self.postMessage({ type: 'status', message: `AI Model: Insufficient history (${validHistory.length}) for meaningful LSTM sequence (need ${SEQUENCE_LENGTH}).` });
        return { xs: null, ys: null, scaler: null, featureCount: 0 };
    }
    ...
    // Changed loop condition to ensure enough history for SEQUENCE_LENGTH of data AND a target
    for (let i = 0; i <= validHistory.length - (SEQUENCE_LENGTH + 1); i++) {
...
async function trainEnsemble(historyData, historicalStreakData) {
    ...
    if (!xs || !ys.group_output || !ys.failure_output || !ys.streak_output) { // Ensure all outputs are valid
        self.postMessage({ type: 'status', message: 'AI Ensemble: Not enough valid data to train with all outputs.' });
        isTraining = false;
        return;
    }
...
// Prediction function (updated for third output and robust return)
async function predictWithEnsemble(historyData) {
    const defaultGroupProbs = allPredictionTypes.reduce((acc, type) => ({ ...acc, [type.id]: 0 }), {});
    const defaultFailureProbs = failureModes.reduce((acc, mode) => ({ ...acc, [mode]: 0 }), {});
    const defaultStreakPreds = allPredictionTypes.reduce((acc, type) => ({ ...acc, [type.id]: 0 }), {});

    const activeModels = ensemble.filter(m => m.model && m.scaler);
    if (activeModels.length === 0) {
        return { groups: defaultGroupProbs, failures: defaultFailureProbs, streakPredictions: defaultStreakPreds };
    }

    const validHistory = historyData.filter(item => item.status === 'success' && item.winningNumber !== null);
    if (validHistory.length < SEQUENCE_LENGTH) {
        // Not enough successful history for a valid sequence prediction
        return { groups: defaultGroupProbs, failures: defaultFailureProbs, streakPredictions: defaultStreakPreds };
    }
    ...
    const scaleFeature = (value, index) => {
        if (!scaler) return value; // Should not happen if activeModels.length > 0
