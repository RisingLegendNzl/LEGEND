        ...
        case 'start':
            ...
            simulationHelpers = {
                ...
                calculateTrendStats: new Function('currentHistory', 'current_STRATEGY_CONFIG', 'activeTypes',
                    ...
                    return (${getFunctionBody(payload.helpers.calculateTrendStats)})
                ...
                getBoardStateStats: new Function('simulatedHistory', 'current_STRATEGY_CONFIG',
                    ...
                    return (${getFunctionBody(payload.helpers.getBoardStateStats)})
                ...
                runNeighbourAnalysis: new Function('render', 'simulatedHistory', 'current_STRATEGY_CONFIG', 'current_useDynamicTerminalNeighbourCount',
                    ...
                    const getNeighboursFunc = this.getNeighbours; // Pass getNeighbours
                    return (${getFunctionBody(payload.helpers.runNeighbourAnalysis)})
                    `).bind(payload.helpers),

                // getRecommendation(trendStats, boardStats, neighbourScores, diff, inputNum1, inputNum2, isForWeightUpdate, aiPredictionData, currentAdaptiveInfluences, lastWinningNumber, useProximityBoostBool, useWeightedZoneBool, useNeighbourFocusBool, isAiReadyBool, useTrendConfirmationBool, current_STRATEGY_CONFIG, current_ADAPTIVE_LEARNING_RATES, currentHistoryForTrend)
                getRecommendation: new Function(
                    ...
                    const terminalMapping = this.terminalMapping;
                    const rouletteWheel = this.rouletteWheel;
                    const getHitZoneFunc = this.getHitZone; // Pass dependencies
                    const getNeighboursFunc = this.getNeighbours;
                    const calculatePocketDistanceFunc = this.calculatePocketDistance;
                    const useDynamicTerminalNeighbourCount = this.useDynamicTerminalNeighbourCount; // Fixed toggle for optimizer
                    const DEBUG_MODE = false; // Always false in worker for performance, logs removed by build process
                    
                    // Re-implement the nested helper calls to use the passed functions
                    const getHitZone = (baseNum, terminals, winningNum, dynTermNeighCount, rouletteWheelArr, termMap, getNeighboursF) => 
                        getHitZoneFunc(baseNum, terminals, winningNum, dynTermNeighCount, rouletteWheelArr, termMap, getNeighboursF);
                    
                    const calculatePocketDistance = (num1, num2, rouletteWheelArr) => 
                        calculatePocketDistanceFunc(num1, num2, rouletteWheelArr);

                    // The actual body of getRecommendation
                    return (${getFunctionBody(payload.helpers.getRecommendation)})
                    `).bind(payload.helpers),
                ...
                getHitZone: new Function('baseNumber', 'terminals', 'winningNumber', 'useDynamicTerminalNeighbourCountBool', 'rouletteWheelArr', 'terminalMappingObj', 'getNeighboursFunc',
                    `
                    const terminalMapping = terminalMappingObj; // Use passed map
                    const rouletteWheel = rouletteWheelArr;     // Use passed wheel
                    const getNeighbours = getNeighboursFunc;    // Use passed func
                    const useDynamicTerminalNeighbourCount = useDynamicTerminalNeighbourCountBool; // Use passed toggle
                    return (${getFunctionBody(payload.helpers.getHitZone)})
                    `),
                ...
                getNeighbours: new Function('number', 'count', 'rouletteWheelArr',
                    `
                    const rouletteWheel = rouletteWheelArr; // Use passed wheel
                    return (${getFunctionBody(payload.helpers.getNeighbours)})
                    `),
                ...
                calculatePocketDistance: new Function('num1', 'num2', 'rouletteWheelArr',
                    `
                    const rouletteWheel = rouletteWheelArr; // Use passed wheel
                    return (${getFunctionBody(payload.helpers.calculatePocketDistance)})
                    `),
                ...
                evaluateCalculationStatus: new Function('historyItem', 'winningNumber', 'current_STRATEGY_CONFIG', 'useDynamicTerminalNeighbourCountBool',
                    ...
                    const terminalMapping = this.terminalMapping;
                    const rouletteWheel = this.rouletteWheel;
                    const getHitZoneFunc = this.getHitZone;
                    const getNeighboursFunc = this.getNeighbours;
                    const useDynamicTerminalNeighbourCount = useDynamicTerminalNeighbourCountBool;
                    const DEBUG_MODE = false;

                    const getHitZone = (baseNum, terminals, winningNum, dynTermNeighCount, rouletteWheelArr, termMap, getNeighboursF) => 
                        getHitZoneFunc(baseNum, terminals, winningNum, dynTermNeighCount, rouletteWheelArr, termMap, getNeighboursF);
                    
                    return (${getFunctionBody(payload.helpers.evaluateCalculationStatus)})
                    `).bind(payload.helpers),
            };
